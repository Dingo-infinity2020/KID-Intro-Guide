...
""").strip()
md = insert_before_once(md, "## 2.1 先看全局：KID 是一台“两种频率协同工作”的机器", ch2_md, "第一次读这一章：先看地图")
...
\end{tcolorbox}
""").strip()
tex = insert_before_once(tex, r"\section{先把“电感”拆成两种储能机制}", ch3_tex, "先认清两套语言：这一章为什么突然出现")
...
""").strip()
md = insert_before_once(md, "## 3.1 先把“电感”拆成两种储能机制", ch3_md, "先认清两套语言")
...
\end{tcolorbox}
""").strip()
tex = insert_before_once(tex, r"\section{正常金属与超导态：区别不只是“电阻变成零”}", ch4_tex, "在看能带图之前，先把三个词翻成普通话")
...
""").strip()
md = insert_before_once(md, "## 4.2 正常金属与超导态：区别不只是“电阻变成零”", ch4_md, "在看能带图之前，先把三个词翻成普通话")
...
\end{tcolorbox}
""").strip()
tex = insert_before_once(tex, r"\section{为什么需要“复电导”这层语言？}", ch5_tex, "这一章建议读两遍：第一遍只抓住")
...
""").strip()
md = insert_before_once(md, "## 5.1 为什么需要“复电导”这层语言？", ch5_md, "这一章建议读两遍")
...
md_checks = ["第一次读这一章：先看地图", "先认清两套语言", "在看能带图之前，先把三个词翻成普通话", "这一章建议读两遍"]
...
if "Step 3-C" not in existing_audit:
