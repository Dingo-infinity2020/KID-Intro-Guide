...
\hypersetup{colorlinks=true,linkcolor=KidBlue,urlcolor=KidBlue,citecolor=KidTeal,pdftitle={KID入门讲义 v0.9},pdfauthor={ChatGPT 辅助整理}}
\setstretch{1.15}
...
\vspace{1.4cm}
{\large Version 0.9 · 2026-09-08}\par
\vfill
...
v0.8 是第一次“阶段性停靠”：不再继续堆叠新理论，而是把 v0.1--v0.7 压缩成一套可手写、可闭卷复述的理论闭环：
\begin{itemize}
  \item 用一张总图重建 $h\nu\rightarrow N_{\rm qp}\rightarrow\sigma_1,\sigma_2\rightarrow L_k,Q_i\rightarrow f_0\rightarrow S_{21}\rightarrow I/Q\rightarrow$ NEP；
  \item 用九张核心笔记卡复习 pair breaking、动能电感、复电导、$Q$、IQ circle、fixed-tone readout、responsivity 与 LEKID 双频电磁身份；
  \item 增加“公式三问”“公式阶梯”和 15 道闭卷概念题，迫使读者从数学、物理、工程三层理解公式；
  \item 增加一个从 \SI{150}{GHz} photon 一路计算到 IQ 位移的综合闭环题；
  \item 单独设置详细答案章，逐步给出推导、单位检查、物理解释、工程意义与常见误区。
\end{itemize}
当前 v0.9 则把已经串起来的理论逐项映射到真实器件的 Sonnet--CST--加工--低温 $S_{21}$--光学标定验证矩阵，并把“一个结果怎样成为可复现证据”写成可直接执行的项目流程。
...
\begin{projectbox}
你现在采用 hairpin / half-hairpin + wiggle 的思路，本质上就是在同时优化四件事：\textbf{active volume、毫米波有效阻抗、GHz 电感、偏振纯度}。所以以后不要把“wiggle 长度”只当成机械几何参数；它应该进入 optical + microwave 两套 sweep。
\end{projectbox}
...
\begin{projectbox}
这正好解释你当前 CST 验证中为什么反射/散射能量闭合不能只看 TE$_{11}$ 的两个偏振分量：在 \SI{150}{GHz}、$D=\SI{1.6}{mm}$ 的圆波导里，TM$_{01}$ 已经是传播通道。若忽略它，$|S|^2$ 能量闭合可能看起来“凭空丢失”。
\end{projectbox}
...
\begin{projectbox}
你当前几何里 X 是 full hairpin、Y 是两个 half-hairpin 串联，而且必须单层、无 crossover。这样的拓扑天然会让两偏振的 end-turn / access path 不完全等价。因此“几何四重对称”真正要验证的是\textbf{电流分布和吸收谱是否恢复近似对称}，而不只是 CAD 轮廓看起来对称。
\end{projectbox}
...
\section{映射到当前双偏振 150 GHz 项目的验证矩阵}
...
\begin{goalbox}
这一章\textbf{不再引入新的核心理论}。目标是把前面 v0.1--v0.7 中已经出现的概念重新压缩、重排，并训练你在不翻书的情况下完成三件事：
\begin{enumerate}
...
如果下面 12 项中有 10 项以上能够不翻书讲清楚，可以认为 v0.1--v0.7 的理论主链已经基本串起来：
\begin{enumerate}
...
\begin{formula}[title=\bfseries v0.8 的一句话]
\centering
\[
\boxed{
\text{真正掌握 KID}
...
\begin{goalbox}
v0.1--v0.8 已经回答了“为什么 KID 能工作”。本章回答另一个真正决定科研是否可靠的问题：\textbf{怎样证明你设计的这个 KID 确实按预期工作，而且别人能够复现这个结论？}

本章把当前双偏振 \SI{150}{GHz} LEKID 项目拆成一条证据链：
\[
\boxed{
\text{设计参数}
\rightarrow
...
v0.9 不要求你一开始就拥有完美材料参数，而要求把材料不确定度\textbf{显式化}。例如对 $L_{k,\square}$、film thickness、$R_\square$ 做上下界 sweep，回答：
\begin{itemize}
...
\]
\end{formula}
这一步把“材料参数来自文献”升级成“材料参数来自你自己的 wafer”。
...
下面是 v0.9 最核心的表。以后每做完一个结果，都应能落到其中一行。
...
\section{v0.9 最终总图：把所有东西压回一条科研证据链}
...
\begin{projectbox}
v0.9 的目标不是让你“多会几个软件”，而是形成一种以后做任何 KID 都能复用的习惯：\textbf{先写 claim，再定义 observable；先过数值 gate，再比较设计；先做 uncertainty，再谈 improvement；最后让实验结果反过来更新模型。}
\end{projectbox}
