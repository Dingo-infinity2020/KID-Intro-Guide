...
v0.8 已经完成阶段性理论复习与手写训练；当前 v0.9 不再增加新的基础理论，而是把已有知识变成可执行的项目验证链：
...
# 第 6 章：微波谐振器与 IQ 读出——从 Q 到可拟合的 S21
