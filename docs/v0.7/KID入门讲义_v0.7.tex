...
\hypersetup{colorlinks=true,linkcolor=KidBlue,urlcolor=KidBlue,citecolor=KidTeal,pdftitle={KID入门讲义 v0.7},pdfauthor={ChatGPT 辅助整理}}
\setstretch{1.15}
...
\vspace{1.4cm}
{\large Version 0.7 · 2026-09-07}\par
\vfill
...
当前版本 v0.7 在 v0.6 的基础上继续推进 LEKID 电磁设计：
\begin{itemize}
  \item 完整保留 v0.1--v0.6 的材料、谐振器、IQ、responsivity 与 NEP 主链；
  \item 建立“同一 meander 在 150 GHz 是 absorber、在 GHz 是 kinetic inductor”的双频电磁图景；
  \item 引入 fill factor、effective sheet impedance、absorber volume 与 optical matching 的工程权衡；
  \item 系统讲解 circular-waveguide cutoff、TE$_{11}$ 简并、TM$_{01}$ 传播通道与偏振 basis；
  \item 从 transmission-line toy model 解释 quarter-wave backshort，并说明真实最优厚度为何偏离 $\lambda/4$；
  \item 把 IDC、coupling capacitor、$Q_c$、cross-pol、Sonnet/CST 分工和 full-wave power closure 接入同一验证矩阵。
\end{itemize}
后续 v0.8 将把这些设计原则逐项映射成当前双偏振 \SI{150}{GHz} LEKID 的理论--Sonnet--CST--实验验证矩阵。
...
\begin{center}
\begin{tabularx}{0.96\textwidth}{>{\bfseries}p{28mm}XX}
\toprule
...
\begin{itemize}
  \item v0.8：把 v0.1--v0.7 理论逐项映射到当前双偏振 \SI{150}{GHz} LEKID 项目，形成可执行的 Sonnet/CST/实验验证矩阵；
  \item v0.9+：阵列频分复用、readout budget、resonance collision、实时 RFSoC / FPGA / GPU 读出与完整 instrument closure。
\end{itemize}
