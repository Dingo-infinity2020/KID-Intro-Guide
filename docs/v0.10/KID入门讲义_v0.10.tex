...
\hypersetup{colorlinks=true,linkcolor=KidBlue,urlcolor=KidBlue,citecolor=KidTeal,pdftitle={KID入门讲义 v0.10},pdfauthor={KID Intro Guide project}}
\setstretch{1.15}
...
\vspace{1.4cm}
{\large Version 0.10 · 2026-09-10}\par
\vfill
...
$n_{\rm qp}$ & 准粒子数密度 & 单位体积内的准粒子数；若体积为 $V$ 且分布近似均匀，则 $N_{\rm qp}\approx n_{\rm qp}V$。 \\
...
$P_{\rm inc}$ & 入射光功率 & 到达器件/定义参考面的光功率；通常有 $P_{\rm abs}\le P_{\rm inc}$。 \\
$P_{\rm abs}$ & 吸收光功率 & 真正进入 absorber 并转化为探测器激发的功率。 \\
$P_{\rm read}$ & 微波读出功率 & GHz probe tone 的功率；与被探测的毫米波/亚毫米波光功率不是同一个量。 \\
...
$Z_s=R_s+iX_s$ & 表面阻抗 & 把有限损耗与感性响应放在同一个复数材料参数里。 \\
\bottomrule
\end{longtable}
\normalsize
...
\section{工程案例：\SI{150}{GHz} 双偏振 LEKID 的材料判据}

\begin{projectbox}
对于这一 \SI{150}{GHz} 工程案例，建议每次谈材料或仿真参数时都同时写出四个量：
\[
T_c,\qquad
\Delta_0,\qquad
...
\section{工程案例的系统边界：不要把所有东西塞进一个模型}

该工程案例的像素同时存在两个相差约两个数量级的频率域：
\[
...
\begin{projectbox}
对双偏振 LEKID 工程案例，一个很有用的建模原则是：\textbf{Sonnet 负责把 GHz resonator 做对；CST/HFSS 负责把 150 GHz optical coupling 做对；低温测试负责告诉你真实材料参数与模型偏差到底在哪里。}
\end{projectbox}
...
\midrule
\endhead
Optical port & $D_{wg}$ & 圆波导名义直径，例如 \SI{1.6}{mm}；必须标注 reference plane \\
Optical band & $f_{opt}$ & 例如 149--151 GHz 可作为窄带验证窗口，不应自动等同最终 science band \\
...
在任何 expensive solver 之前先做静态审计。本工程案例的双偏振单层结构至少要检查：
\begin{enumerate}
...
以 $D=\SI{1.6}{mm}$ 圆波导、150 GHz 附近为例，TE$_{11}$ 已传播，TM$_{01}$ 也已超过截止，而 TE$_{21}$ 仍截止。因此若只看 TE$_{11}\rightarrow$TE$_{11}$，就可能把转换到 TM$_{01}$ 的功率误认为吸收。

\begin{formula}[title=工程案例窄带验证的 modal accounting]
\[
\boxed{
P_{refl}
...
该工程问题的价值不在“B1 看起来更复杂”，而在它能否在不牺牲其他性能的前提下带来可重复的收益。
