...
\hypersetup{colorlinks=true,linkcolor=KidBlue,urlcolor=KidBlue,citecolor=KidTeal,pdftitle={KID入门讲义 v0.8},pdfauthor={ChatGPT 辅助整理}}
\setstretch{1.15}
...
\vspace{1.4cm}
{\large Version 0.8 · 2026-09-08}\par
\vfill
...
当前版本 v0.8 是第一次“阶段性停靠”：不再继续堆叠新理论，而是把 v0.1--v0.7 压缩成一套可手写、可闭卷复述的理论闭环：
\begin{itemize}
...
\end{itemize}
后续 v0.9 再把已经串起来的理论逐项映射到真实器件的 Sonnet--CST--加工--低温 $S_{21}$--光学标定验证矩阵。
...
\begin{itemize}
  \item v0.9：把 v0.1--v0.8 理论逐项映射到当前双偏振 \SI{150}{GHz} LEKID 项目，形成可执行的 Sonnet/CST/加工/低温 $S_{21}$/光学标定验证矩阵；
  \item v1.0+：阵列频分复用、readout budget、resonance collision、实时 RFSoC / FPGA / GPU 读出与完整 instrument closure。
\end{itemize}
